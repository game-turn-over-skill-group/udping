...
def warm_up_connection(client, resolved_target_host, target_port, proxy):
    ...
    if not proxy:
        return
    try:
        client.settimeout(0.05)  # 预热包固定等待0.05秒
        # 发送一个预热数据包并忽略其响应
...
def clear_socket_buffer(client, show_debug):
    ...
    try:
        client.settimeout(0.05)  # 设置一个非常短的超时
        while True:
            try:
        ...
        if continuous:  # 在持续发送模式下
            while True:
            	...
            	if args.listen_port == 0 and sent_packets >= 4:
            		...
            		warm_up_connection(client, resolved_target_host, target_port, proxy)  # 预热新端口连接 = 防止首个包高延迟
            		sent_packets = 0
            		#clear_socket_buffer(client, show_debug)  # （非必要）新增：端口切换后强制清空缓冲区，避免旧数据干扰 = 似乎导致控制台刷新卡顿
            ...
            try:
                ...
                clear_socket_buffer(client, show_debug)  # 发送数据包之前清理缓冲区 = 似乎导致控制台刷新卡顿
                client.sendto(data, (resolved_target_host, target_port))
                start_time = time.time()
            ...
            if args.listen_port == 0 and sent_packets >= 4:
            	...
            	if not client:
            		return
            	sent_packets = 0
            	#clear_socket_buffer(client, show_debug)  # 新增：端口切换后强制清空缓冲区，避免旧数据干扰 = 似乎导致控制台刷新卡顿
