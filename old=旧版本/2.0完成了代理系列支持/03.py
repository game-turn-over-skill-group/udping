import socket
import time
import sys
from datetime import datetime
import socks  # 需要安装 pysocks: pip install pysocks
import urllib.parse

def udp_tracker(target_host, target_port, hex_data_packets, listen_port=None, ipv6=False, show_timestamp=False, continuous=False, wait_time=1000, proxy=None):
    try:
        while True:
            # 判断是否使用代理
            if proxy:
                # 如果使用代理，并根据是否为IPv6决定创建的socket类型
                if ipv6:
                    client = socks.socksocket(socket.AF_INET6, socket.SOCK_DGRAM)  # 创建一个支持IPv6的 UDP代理socket
                else:
                    client = socks.socksocket(socket.AF_INET, socket.SOCK_DGRAM)  # 创建一个支持IPv4的 UDP代理socket
                # 配置代理设置
                proxy_type, proxy_host, proxy_port = proxy.split(':')
                if proxy_type.lower() == 'socks5':
                    client.set_proxy(socks.SOCKS5, proxy_host, int(proxy_port))
                elif proxy_type.lower() == 'http':
                    client.set_proxy(socks.HTTP, proxy_host, int(proxy_port))
                # 连接代理服务器
                client.connect((proxy_host, int(proxy_port)))
            else:
                # 不使用代理的情况下创建普通socket
                if ipv6:  # 根据ipv6标志创建socket
                    client = socket.socket(socket.AF_INET6, socket.SOCK_DGRAM)  # 创建一个IPv6的UDP socket
                else:
                    client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # 创建一个IPv4的UDP socket

            # 如果设置了代理
            if proxy:
            	parsed_proxy = urllib.parse.urlparse(proxy)  # 解析代理URL
            	proxy_type = socks.SOCKS5 if parsed_proxy.scheme == 'socks' else socks.HTTP  # 根据URL的scheme选择代理类型
            	proxy_host = parsed_proxy.hostname  # 获取代理主机地址
            	proxy_port = parsed_proxy.port  # 获取代理端口
            	client.set_proxy(proxy_type, proxy_host, proxy_port)  # 设置代理

            # 如果设置了固定监听端口，绑定到该端口
            if listen_port:
                if ipv6:
                    client.bind(("::", listen_port))  # 在IPv6下绑定到指定端口
                else:
                    client.bind(("0.0.0.0", listen_port))  # 在IPv4下绑定到指定端口

            # 设置等待超时
            client.settimeout(wait_time / 1000)  # 将等待时间从毫秒转换为秒，并设置socket超时时间

            # 获取目标地址的解析结果
            try:
                resolved_target_host = socket.getaddrinfo(target_host, None, socket.AF_INET6 if ipv6 else socket.AF_INET)[0][4][0]
            except socket.gaierror as e:
                print(f"Failed to resolve target host {target_host}: {e}")
                return

            # 循环发送数据包并接收响应
            for i, hex_data in enumerate(hex_data_packets):
                byte_data = bytes.fromhex(hex_data)  # 将十六进制字符串转换为字节数据
                try:
                    # 如果有-s参数，显示系统时间
                    if show_timestamp:
                        print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")  # 打印当前时间戳
                    # 发送十六进制数据
                    client.sendto(byte_data, (resolved_target_host, target_port))  # 向目标主机发送UDP数据包
                    # 开始时间
                    start_time = time.time()  # 记录开始时间
                    # 试图接收响应
                    data, addr = client.recvfrom(4096)  # 接收来自目标主机的响应数据包
                    # 结束时间
                    end_time = time.time()  # 记录结束时间
                    # 计算延迟
                    delay = (end_time - start_time) * 1000  # 将延迟从秒转换为毫秒
                    # 打印数据包类型或编号以及反馈信息
                    print(f"Type[{i+1:02d}]: Received response from {addr} with delay of {delay:.6f} ms")  # 打印接收的响应信息
                except socket.timeout:
                    local_port = client.getsockname()[1]  # 获取本地监听端口
                    print(f"UDP request timed out (Target IP: {resolved_target_host}, Target Port: {target_port}, Local Port: {local_port})")
                except Exception as e:
                    print(f"An error occurred: {e}")  # 打印其他异常信息

                # 指定每次发送后的等待时间
                time.sleep(wait_time / 1000)  # 将等待时间从毫秒转换为秒

            # 关闭当前的socket
            client.close()

            # 检测是否需要连续执行
            if not continuous:
                break  # 如果不连续执行，退出循环
            
    finally:
        client.close()  # 确保在异常情况下关闭socket

def is_ipv6_address(hostname):
    try:
        # 解析主机名的地址信息
        addr_info = socket.getaddrinfo(hostname, None)
        # 检查是否存在 IPv6 地址信息
        for info in addr_info:
            if info[0] == socket.AF_INET6:  # 检查是否为IPv6地址
                return True
    except socket.gaierror:
        pass
    return False

def is_ipv4_address(hostname):
    try:
        # 解析主机名的地址信息
        addr_info = socket.getaddrinfo(hostname, None)
        # 检查是否存在 IPv4 地址信息
        for info in addr_info:
            if info[0] == socket.AF_INET:  # 检查是否为IPv4地址
                return True
    except socket.gaierror:
        pass
    return False

if __name__ == "__main__":
    # 命令行参数解析
    if len(sys.argv) < 2:
        print("Usage: python udp_tracker.py [-4 | -6] <domain> [<port>] [-s] [-t <wait_time_ms>] [-p <listen_port>] [-x <proxy>]")
        sys.exit(1)

    # 检查是否有 -4 或 -6 选项, 以手动指定IPv4或IPv6 (包含删除参数避免下面定义错误)
    ipv6 = False
    if '-4' in sys.argv:
        ipv6 = False
        sys.argv.remove('-4')
    elif '-6' in sys.argv:
        ipv6 = True
        sys.argv.remove('-6')
    else:
        # 如果未指定 -4 或 -6 参数，则根据域名的类型选择 IP 模式
        if is_ipv4_address(sys.argv[1]):
            ipv6 = False  # 默认IPv4
        elif is_ipv6_address(sys.argv[1]):
            ipv6 = True  # 默认IPv6

    # 定义解析命令行参数
    target_host = sys.argv[1]  # 目标域名或IP地址
    
    # 检查第2个参数是否为整数，如果不是或为空，使用默认端口号6969
    target_port = int(sys.argv[2]) if len(sys.argv) >= 3 and sys.argv[2].isdigit() else 6969
    
    show_timestamp = '-s' in sys.argv  # 是否显示时间戳
    continuous = '-t' in sys.argv  # 是否连续执行

    # 检查是否指定了自定义等待时间
    wait_time = 1000  # 默认等待时间为1000毫秒
    wait_time_index = sys.argv.index('-t') + 1 if '-t' in sys.argv and len(sys.argv) > sys.argv.index('-t') + 1 else -1
    if wait_time_index != -1 and wait_time_index < len(sys.argv):
        wait_time = int(sys.argv[wait_time_index])  # 设置等待时间
    
    # 检查是否指定了监听端口
    listen_port = None
    listen_port_index = sys.argv.index('-p') + 1 if '-p' in sys.argv and len(sys.argv) > sys.argv.index('-p') + 1 else -1
    if listen_port_index != -1 and listen_port_index < len(sys.argv):
        listen_port = int(sys.argv[listen_port_index])  # 设置本地监听端口

    # 检查是否指定了代理
    proxy = None
    proxy_index = sys.argv.index('-x') + 1 if '-x' in sys.argv and len(sys.argv) > sys.argv.index('-x') + 1 else -1
    if proxy_index != -1 and proxy_index < len(sys.argv):
        proxy = sys.argv[proxy_index]  # 设置代理地址

    # 定义固定的封包内容
    hex_data_packets = [
        # 连接
        '000004172710198000000000697CD3FA',
        # 开始
        '',
        # 更新
        '',
        # 断开
        ''
    ]

    # 调用函数执行请求
    udp_tracker(target_host, target_port, hex_data_packets, listen_port=listen_port, ipv6=ipv6, show_timestamp=show_timestamp, continuous=continuous, wait_time=wait_time, proxy=proxy)
