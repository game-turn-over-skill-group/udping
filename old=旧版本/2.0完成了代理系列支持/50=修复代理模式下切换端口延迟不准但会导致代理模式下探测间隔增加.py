...
def create_socket_and_bind(protocol, proxy_type, proxy_host, proxy_port, listen_port, show_debug):
    ...
    try:
        # 核心修复1：SOCKS5代理不支持IPv6，强制使用IPv4 socket
        if proxy_type == 'socks':
            if show_debug:
                print("Creating SOCKS5 Proxy socket (IPv4 only)...")
            # SOCKS5 UDP代理仅支持AF_INET
            client = socks.socksocket(socket.AF_INET, socket.SOCK_DGRAM)
            client.set_proxy(socks.SOCKS5, proxy_host, proxy_port)
            # 强制覆盖协议为IPv4，避免SOCKS5+IPv6冲突
    ...
    try:
        ...
        if protocol == socket.AF_INET6:
            ...
            # 核心修复2：IPv6绑定简化为双元组（部分系统/库不支持四元组）
            client.bind(("::", listen_port))  
            sockname = client.getsockname()
            # 解析IPv6 sockname（四元组转端口）
        ...
        if show_debug:
            print(f"Listening on port {bind_port}... ", end="")
            if bind_port == listen_port or listen_port == 0:  # 随机端口时不校验
                print("ok")
            else:
                print("fail")
        return client, bind_port  # 返回实际绑定的端口
    except Exception as e:
...
def warm_up_connection(client, resolved_target_host, target_port, proxy):
    ...
    if not proxy:
        return
    try:
        client.settimeout(args.wait_time)  # 预热包固定等待?秒 默认wait_time超时时间
        # 发送一个预热数据包并忽略其响应
...
def clear_socket_buffer(client, show_debug):
    ...
    try:
        client.settimeout(args.interval_time)  # 设置超时时间 默认interval_time间隔时间
        while True:
            try:
...
def udp_tracker(target_host, target_port, custom_hex_data, is_custom_hex, listen_port, use_ipv4, use_ipv6, show_debug, continuous, interval_time, wait_time, proxy):
    ...
    if proxy_type == 'socks' and use_ipv6:
    ...
    try:
        if continuous:  # 在持续发送模式下
            while True:
            	try:
            		...
            		with lock:  # 线程安全地清理缓冲区并发送数据包
            			...
            			if proxy:
            				clear_socket_buffer(client, show_debug)  # 发送数据包之前清理缓冲区（解决代理延迟不准）
            	...
            	if args.listen_port == 0 and sent_packets >= 4:
            		...
            		sent_packets = 0
            		#if proxy:
            			#clear_socket_buffer(client, show_debug)  # 新增：端口切换后强制清空缓冲区（解决代理延迟不准）

            	#precise_sleep(interval_time)  # 新版本延迟控制 似乎是导致CPU高占用 元凶
            ...
            try:
                ...
                if proxy:
                	clear_socket_buffer(client, show_debug)  # 发送数据包之前清理缓冲区 = 连续包中使用会导致控制台刷新卡顿
                client.sendto(data, (resolved_target_host, target_port))
                start_time = time.time()
            ...
            if args.listen_port == 0 and sent_packets >= 4:
            	...
            	if not client:
            		return
            	sent_packets = 0
            	#clear_socket_buffer(client, show_debug)  # 新增：端口切换后强制清空缓冲区，避免旧数据干扰 = 连续包中使用会导致控制台刷新卡顿
