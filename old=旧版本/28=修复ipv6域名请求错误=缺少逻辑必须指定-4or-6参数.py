...
if __name__ == "__main__":
    ...
    # Check if both IPv4 and IPv6 are specified
    if args.ipv4 and args.ipv6:
        print("Error: Cannot specify both IPv4 and IPv6. Choose only one.")
        sys.exit(1)

    # Check if neither IPv4 nor IPv6 is specified
    if not args.ipv4 and not args.ipv6:
        print("Error: Either IPv4 (-4) or IPv6 (-6) must be specified.")
        sys.exit(1)

    # Set default protocol if not specified
    protocol = socket.AF_INET6 if args.ipv6 else socket.AF_INET
