...
def warm_up_connection(client, resolved_target_host, target_port, proxy):
    """发送预热包打通 SOCKS5 UDP 路径，不等待回复（避免占用 wait_time 影响延迟准确性）。
    后续 clear_socket_buffer 会在发包前丢弃掉预热包的响应（如果有的话）。"""
    if not proxy:
        return
    try:
...
def print_statistics(resolved_host, target_port, stats):
    ...
    if times:
        ...
        print(f"     Minimum = {mn:.3f}ms, Maximum = {mx:.3f}ms, Average = {avg:.3f}ms")
    else:
        print(f"     {succeeded} successful, {failed} failed.  ({fail_pct:.2f}% fail)")
...
def udp_tracker(target_host, target_port, custom_hex_data, is_custom_hex, listen_port,
    ...
    # 预热包发出后不等待，交给发包前的 clear_socket_buffer 清除残留响应
    count = 0
    sent_packets = 0
    stats = {'sent': 0, 'succeeded': 0, 'failed': 0, 'times': []}

    # 连续模式或 -n 次数模式都用同一个循环，统一控制
    if continuous:
    ...
    def do_one_probe():
        """发送一个探测包，返回是否成功"""
        nonlocal count, sent_packets, client, listen_port
        count += 1
        sent_packets += 1
        try:
            if is_custom_hex:
    ...
    try:
        ...
        while keep_going:
            ...
            # 随机端口模式下，每4包切换一次端口（重做 SOCKS5 握手）
            if listen_port == 0 and sent_packets >= 4:
                client.close()
                with lock:
        ...
        pass  # Ctrl+C 正常退出，统计在 finally 打印
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--count", type=int, default=1, help="Number of probes to send (default: 1; use -c for unlimited)")
    ...
    if args.continuous and args.count != 1:
        print("Note: -c (continuous) takes precedence over -n (count).")
