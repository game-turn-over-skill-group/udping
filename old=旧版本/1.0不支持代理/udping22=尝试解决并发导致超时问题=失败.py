...
def udp_tracker(target_host, target_port, hex_data_packets, ipv6=False, show_timestamp=False, continuous=False, initial_wait_time=1000, increment_wait_time=100):
    try:
        # 根据ipv6标志创建socket
        if ipv6:
        ...
        for i, hex_data in enumerate(hex_data_packets):
            ...
            # 更新等待时间
            wait_time += increment_wait_time

        # 检测是否需要连续执行
        if continuous:
            # 递归调用时传递正确的参数
            udp_tracker(target_host, target_port, hex_data_packets, ipv6=ipv6, show_timestamp=show_timestamp, continuous=continuous, initial_wait_time=initial_wait_time, increment_wait_time=increment_wait_time)
        else:
            # 如果不需要连续执行，退出递归
            return
...
if __name__ == "__main__":
    ...
    # 执行UDP探测
    udp_tracker(target_host, target_port, hex_data_packets, ipv6=ipv6, show_timestamp=show_timestamp, continuous=continuous, initial_wait_time=wait_time)
