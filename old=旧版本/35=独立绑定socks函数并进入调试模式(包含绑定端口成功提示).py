...
def create_socket_and_bind(protocol, proxy_type, proxy_host, proxy_port, listen_port):
	print(f"Creating socket with protocol: {protocol}, proxy_type: {proxy_type}, proxy_host: {proxy_host}, proxy_port: {proxy_port}, listen_port: {listen_port}")
	try:
		if proxy_type == 'socks':
	...
	try:
		...
		if protocol == socket.AF_INET6:
			...
			print(f"Socket bound to {client.getsockname()}")

			# Additional debugging
			sockname = client.getsockname()
			print(f"Getsockname returned: {sockname}")

			if sockname[1] == listen_port:
				print("Binding successful")
			else:
				print("Binding failed, port mismatch")
		else:
